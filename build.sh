...
mysqldump -e --hex-blob -hdb --add-drop-database --add-drop-table --add-drop-trigger -u"${MYSQL_USER}" -p"${MYSQL_PASSWORD}" "${MYSQL_DATABASE}" > "${backupFolder}"/fulldb.sql && echo "Success"
